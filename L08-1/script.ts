...
namespace L08 {
    ...
    let colors: string [] = ["red", "blue", "green", "darkgreen", "lightgreen", "orange", "lightblue", "darkblue", "crimson", "cyan"];
    ...
    function generateArt(): void {
        canvas.height = 600;
        canvas.width = 1200;

        generateBackground();

        for (let index: number = 0; index < 1 + randomNumber(100); index++) {
            generateBackground();
    ...
    function generateBackground(): void {
        let gradient: CanvasGradient = crc2.createLinearGradient(0, 0, 1000, 300);
    ...
    function generateCircle(_index: number): void {
        for (_index = 0; _index < 150; _index++) {
            let x: number = Math.floor(Math.random() * 1200);
            var y: number = Math.floor(Math.random() * 600);
    ...
    function generateTriangle(_index: number): void {
    ...
    function generarteRectangle(_index: number): void {
        for (_index = 0; _index < 5; _index++) {
            let x: number = Math.floor(Math.random() * 1200);
            var y: number = Math.floor(Math.random() * 600);
    ...
    function randomNumber (_multiplicator: number): number {
