"use strict";
/*
 Aufgabe:<L10.2_Vogelhaus>
 Name: <Daniel Meier>
 Matrikel: <271129>
 Datum: <14.01.2023>
 Quellen: <Natan Haider>
*/
var L10_Vogelhaus;
(function (L10_Vogelhaus) {
    window.addEventListener("load", handleLoad);
    L10_Vogelhaus.golden = 0.62;
    let snowflakesPos = new L10_Vogelhaus.Vector(400, 200);
    let snowflakes = [];
    function handleLoad() {
        let canvas = document.querySelector("canvas");
        if (!canvas)
            return;
        L10_Vogelhaus.crc2 = canvas.getContext("2d");
        L10_Vogelhaus.drawStatic();
        L10_Vogelhaus.imgData = L10_Vogelhaus.crc2.getImageData(0, 0, L10_Vogelhaus.crc2.canvas.width, L10_Vogelhaus.crc2.canvas.height);
        drawSnowflakes(50, snowflakesPos);
        drawBirds();
        drawFlyingBirds();
        window.setInterval(update, 20);
    }
    function drawSnowflakes(_nFlakes, _position) {
        let transform = L10_Vogelhaus.crc2.getTransform();
        L10_Vogelhaus.crc2.translate(_position.x, _position.y);
        for (let drawn = 0; drawn < _nFlakes; drawn++) {
            L10_Vogelhaus.crc2.save();
            let pos = new L10_Vogelhaus.Vector(randomNumber(0, 325), randomNumber(0, 250));
            let size = new L10_Vogelhaus.Vector(10, 10);
            let snowflake = new L10_Vogelhaus.Snowflake(pos, size);
            snowflake.draw();
            snowflakes.push(snowflake);
            L10_Vogelhaus.crc2.restore();
        }
        L10_Vogelhaus.crc2.setTransform(transform);
    }
    function updateSnowflakes(_position) {
        let transform = L10_Vogelhaus.crc2.getTransform();
        L10_Vogelhaus.crc2.translate(_position.x, _position.y);
        for (let snow of snowflakes) {
            snow.letItSnow(1 / 100);
            snow.draw();
        }
        L10_Vogelhaus.crc2.setTransform(transform);
    }
    function drawBirds() {
        console.log("Bird");
    }
    function updateBirds() {
        console.log();
    }
    function drawFlyingBirds() {
        console.log("FlyingBirds");
    }
    function updateFlyingBirds() {
        console.log();
    }
    function update() {
        L10_Vogelhaus.crc2.putImageData(L10_Vogelhaus.imgData, 0, 0);
        updateSnowflakes(snowflakesPos);
        updateBirds();
        updateFlyingBirds();
    }
    function randomNumber(_min, _max) {
        let returnNumber = Math.floor(Math.random() * (_max - _min)) + _min;
        return returnNumber;
    }
    L10_Vogelhaus.randomNumber = randomNumber;
})(L10_Vogelhaus || (L10_Vogelhaus = {}));
//# sourceMappingURL=main.js.map